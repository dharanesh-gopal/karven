// scripts/populate-careers-jobs.js
// This script populates all job openings into the careers page settings
...
async function populateJobs() {
  try {
    ...
    // Fetch the careers page settings document
    const careersPage = await client.fetch(
      `*[_type == "careersPageSettings"][0]{ _id, _rev }`
    );

    if (!careersPage) {
      console.error('❌ careersPageSettings document not found!');
      console.log('Please create the document in Sanity Studio first.');
      return;
    }

    console.log(`✅ Found careersPageSettings document: ${careersPage._id}`);

    // Update the document with all jobs
    const result = await client
      .patch(careersPage._id)
      .set({
        'jobOpeningsSection.jobOpenings': JOBS,
      })
      .commit();

    console.log(`\n✅ Successfully populated ${JOBS.length} jobs!`);
    console.log('\nJobs added:');
    JOBS.forEach((job, idx) => {
      console.log(`  ${idx + 1}. ${job.title} (${job.department}) - ${job.id.current}`);
    });
    
    console.log('\n🎉 Done! Visit Sanity Studio to see the jobs.');
  } catch (error) {
