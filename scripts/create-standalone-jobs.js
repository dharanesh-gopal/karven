// scripts/create-standalone-jobs.js
// This script creates standalone job opening documents
...
async function createJobs() {
  try {
    console.log('🚀 Creating standalone job documents...\n');

    // Check if jobs already exist
    const existingJobs = await client.fetch(`*[_type == "jobOpening"]`);
    if (existingJobs.length > 0) {
      console.log(`⚠️  Found ${existingJobs.length} existing job(s).`);
      console.log('Deleting existing jobs first...\n');
    ...
    for (const job of JOBS) {
      ...
      results.push(result);
      console.log(`  ✅ Created: ${job.title} (${job.department})`);
    }

    console.log(`\n✅ Successfully created ${results.length} job openings!`);
    console.log('\n🎉 Done! Refresh Sanity Studio to see the jobs.');
  } catch (error) {
