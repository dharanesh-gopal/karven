...
import { ArrowLeft, MapPin, Clock, Briefcase, Upload, CheckCircle, ChevronRight, AlertCircle } from "lucide-react"
...
export default function JobApplicationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!resumeFile) {
      setErrorMessage("Please attach your resume (PDF or DOC, max 5MB).")
      return
    }
    ...
    try {
      ...
      console.error(error)
      setStatus("error")
      setErrorMessage("Unable to send your application right now. Please try again shortly.")
    }
  }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxBytes) {
      setErrorMessage("Resume must be under 5MB.")
      setResumeFile(null)
      return
    }
                ...
                <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-xl shadow-slate-200/50">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="space-y-1">
                        ...
                        <label className="border-2 border-dashed border-slate-300 rounded-xl p-6 flex flex-col items-center justify-center text-center hover:bg-sky-50 hover:border-sky-400 transition-all cursor-pointer group">
                            <input 
                              type="file" 
                              name="resume"
                              accept=".pdf,.doc,.docx"
                            ...
                            {resumeFile ? (
                              ...
                              <p className="text-xs text-slate-500 font-medium">Click to upload PDF or DOC (max 5MB)</p>
                            )}
                        </label>
                    </div>
